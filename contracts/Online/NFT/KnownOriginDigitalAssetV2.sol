 ...
 *https://etherscan.io/token/0xfbeef911dc5821886e1dda71586d90ed28174b7d#code
 */
...
contract AccessControl {
    ...
    modifier onlyIfMinter() {
        require(
            msg.sender == owner ||
                hasRole(msg.sender, ROLE_KNOWN_ORIGIN) ||
    ...
    modifier onlyIfUnderMinter() {
        require(
            msg.sender == owner ||
                hasRole(msg.sender, ROLE_KNOWN_ORIGIN) ||
                hasRole(msg.sender, ROLE_UNDER_MINTER)
        );
        _;
    }
    ...
    function addAddressToAccessControl(
        address _operator,
        uint8 _role
    ) public onlyIfKnownOrigin {
        roles[_role].add(_operator);
        emit RoleAdded(_operator, _role);
    }

    function removeAddressFromAccessControl(
        address _operator,
        uint8 _role
    ) public onlyIfKnownOrigin {
    ...
    Pausable
{
    ...
    uint256 internal constant MAX_UINT32 = ~uint32(0);
    ...
    modifier onlyAvailableEdition(uint256 _editionNumber) {
        require(
            editionNumberToEditionDetails[_editionNumber].totalSupply <
                editionNumberToEditionDetails[_editionNumber].totalAvailable,
            "No more editions left to purchase"
        );
        _;
    }
    ...
    modifier onlyActiveEdition(uint256 _editionNumber) {
        require(
            editionNumberToEditionDetails[_editionNumber].active,
            "Edition not active"
        );
        _;
    }
    ...
    modifier onlyPurchaseDuringWindow(uint256 _editionNumber) {
        require(
            editionNumberToEditionDetails[_editionNumber].startDate <=
                block.timestamp,
            "Edition not available yet"
        );
        require(
            editionNumberToEditionDetails[_editionNumber].endDate >=
                block.timestamp,
    ...
    function _createEdition(
        ...
        require(
            highestEditionNumber.add(
                editionNumberToEditionDetails[highestEditionNumber]
                    .totalAvailable
            ) < _editionNumber,
    ...
    function _updateEditionTypeLookupData(
        ...
        uint256 typeEditionIndex = editionTypeToEditionNumber[_editionType]
            .length;
        editionTypeToEditionNumber[_editionType].push(_editionNumber);
        editionNumberToTypeIndex[_editionNumber] = typeEditionIndex;
    }

    function _updateArtistLookupData(
        address _artistAccount,
        uint256 _editionNumber
    ) internal {
        uint256 artistEditionIndex = artistToEditionNumbers[_artistAccount]
            .length;
    ...
    function _underMintNextTokenId(
        uint256 _editionNumber
        ...
        if (
            _tokenId >
            _editionDetails.editionNumber.add(_editionDetails.totalSupply)
        ) {
            _editionDetails.totalSupply = _editionDetails.totalSupply.add(1);
        }
    ...
    function _handleFunds(
        ...
        CommissionSplit
            storage commission = editionNumberToOptionalCommissionSplit[
                _editionNumber
            ];
        ...
        uint256 remainingCommission = msg.value.sub(artistPayment).sub(
            rateSplit
        );
    ...
    function updateKoCommissionAccount(
        address _koCommissionAccount
    ) external onlyIfKnownOrigin {
    ...
    function updateEditionTokenURI(
        uint256 _editionNumber,
        string _uri
    ...
    function updateArtistCommission(
        uint256 _editionNumber,
        uint256 _rate
    ) external onlyIfKnownOrigin onlyRealEdition(_editionNumber) {
    ...
    function updateArtistsAccount(
        uint256 _editionNumber,
        address _artistAccount
    ) external onlyIfKnownOrigin onlyRealEdition(_editionNumber) {
        EditionDetails
            storage _originalEditionDetails = editionNumberToEditionDetails[
                _editionNumber
            ];
    ...
    function updateEditionType(
        ...
        uint256 newTypeEditionIndex = editionTypeToEditionNumber[_editionType]
            .length;
        editionTypeToEditionNumber[_editionType].push(_editionNumber);
    ...
    function updateTotalSupply(
        uint256 _editionNumber,
        uint256 _totalSupply
    ) public onlyIfKnownOrigin onlyRealEdition(_editionNumber) {
        require(
    ...
    function updateOptionalCommission(
        ...
        editionNumberToOptionalCommissionSplit[
            _editionNumber
        ] = CommissionSplit({rate: _rate, recipient: _recipient});
    }
    ...
    function setTokenURI(
        uint256 _tokenId,
        string _uri
    ) external onlyIfKnownOrigin onlyValidTokenId(_tokenId) {
    ...
    function editionOfTokenId(
        uint256 _tokenId
    ) public view returns (uint256 _editionNumber) {
     ...
     */
    function editionsOfType(
        uint256 _type
    ) public view returns (uint256[] _editionNumbers) {
     ...
     */
    function artistsEditions(
        address _artistsAccount
    ) public view returns (uint256[] _editionNumbers) {
     ...
     */
    function tokensOfEdition(
        uint256 _editionNumber
    ) public view returns (uint256[] _tokenIds) {
    ...
    function editionOptionalCommission(
        uint256 _editionNumber
    ) public view returns (uint256 _rate, address _recipient) {
        CommissionSplit
            storage commission = editionNumberToOptionalCommissionSplit[
                _editionNumber
            ];
    ...
    function tokenURI(
        uint256 _tokenId
    ) public view onlyValidTokenId(_tokenId) returns (string) {
    ...
    function purchaseDatesToken(
        uint256 _tokenId
    ) public view returns (uint256 _startDate, uint256 _endDate) {
        uint256 _editionNumber = tokenIdToEditionNumber[_tokenId];
        return purchaseDatesEdition(_editionNumber);
    }

    function priceInWeiToken(
        uint256 _tokenId
    ) public view returns (uint256 _priceInWei) {
        uint256 _editionNumber = tokenIdToEditionNumber[_tokenId];
    ...
    function artistCommission(
        uint256 _editionNumber
    ) public view returns (address _artistAccount, uint256 _artistCommission) {
        EditionDetails storage _editionDetails = editionNumberToEditionDetails[
            _editionNumber
        ];
        return (
    ...
    function totalAvailableEdition(
        uint256 _editionNumber
    ) public view returns (uint256) {
        EditionDetails storage _editionDetails = editionNumberToEditionDetails[
            _editionNumber
